...
- **Group toggles** turn whole sets of exclusion rules on or off without writing any gitignore syntax
    > Every ignore list and the blocklist is organized into labeled groups (version control, caches, build output, photo libraries, and so on). Flip a switch to enable or disable a group, expand one to toggle individual rules, and *Edit as text* stays one click away for full control
- **Exclude from Index** in the right-click menu drops a result, and optionally every path like it, from the index
    - Smart suggestions recognize common layouts so one rule can cover many files: clear out an app's `Contents/MacOS` subfolders while keeping the executables, skip every `node_modules` or build folder, skip the contents of a Photos or Final Cut library, or drop localization folders
    - A breadcrumb picks how far the rule reaches, from the exact path up to all matching paths
...
- **Per-scope ignore rules** for Applications, System, and the rest of the disk, editable in *Settings*
    > Exclude patterns like every app's `Contents/MacOS` subfolders while keeping the binaries searchable
